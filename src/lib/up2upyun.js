var up2upyun = (function () {
  ...
  function Upload2Upyun(images, files) {
    this.initDefaultConfigs = function () {
      return {
        form_api_secret: 'ibYGXkLBvWPtgPpWL+cZ+Qg7Nrg=',
        params: {
          bucket: 'ces00',
    ...
    this.postIndex = 0;
    this.images = [];
    this.originImages = images ? images : null;
    this.originFiles = files ? files : null;

    var self = this;
    this.configUpalod = function (configs) {
    ...
    this.upload = function (callback, progressCallback) {
      if (!window.JSON)
    ...
    this.doUploadFile = function (file, callback, progressCallback) {
    ...
    this.doUpload = function (blob, callback, progressCallback) {
    ...
    this.dataURLtoBlob = function (dataurl, cb) {
